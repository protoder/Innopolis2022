...
def XGSearchFeatures(SearchFtrX, SearchFtrY, X, Y, ValidX, ValidY, XTest, From = 0, InputMask = None, GPU = 0, UseMetric="mlogloss"):
    ...
    if not os.path.isfile('feature_importances_3.npy'):
        model = XGBClassifier(learning_rate=0.03,
                              n_estimators= 50000,
                              max_depth=6,
                              min_child_weight=6,
        ...
        np.save('feature_importances_3.npy', f)
    else:
        f = np.load('feature_importances_3.npy')

    fInd = np.argsort(f)[::-1]
    CrIndex = fInd.copy()
    LastAccuracy = 0

    if not os.path.isfile('Result3.npy'):
        Result = [0] * len(f)
