...
def TimeAugmentation(XTest, YTest, K, random = None, LevelK=0.05, UseMain = True, Ver = 0, Train = True, SinMode = 0):
    ...
    NewX = [XTest * (np.random.random_sample(size=XTest.shape) * LevelK + 0.975) for i in range(K-(1 if UseMain else 0))]
...
'''
def TestClassify(X, Y, X_test0 = None, y_test0 = None, X_test1 = None, XTestLen = 2071, KAu = 0, N = 175, RandomState = 0, ClsType = 0, Hr = []):
  X_train, y_train = X, Y
...
def ForestHromosom(Hr, X, Y, XTest, WinSize = 100, MaxAuK = 35, MaxAyLevel = 0.3):
    ...
    if MaxAyLevel == 0:
        ...
        X1, Y1 = TimeAugmentation(np.delete(X, DelMask, axis=0),
                                  ...
                                  K= 1 + int(Hr[2] * (MaxAuK-1)), random=int(Hr[3] * 65535), LevelK= 0.01 + Hr[4] * MaxAyLevel, UseMain=Hr[5] < 0.1)
    ...
    if XTest is not None:
        ...
        CrRes, CrValue, CrTest = TestForest2(X1, Y1, Xvalid, Yvalid, XTest, XTestLen, KAu= 0, RandomState=int(Hr[6] * 65535))
...
def GenBoostHr(n_estimators,  # <= 10000 int(Hr[11] * 10000),
              max_depth, # 3..10
              ...
              reg_alpha, #[0..13]
              seed, # <= 65535
              tree_method, # 0..2  индекс в Methods
              KAug, # 1..21
              AugLevel # 0.01 .. 0.313
               ):

    Hr = [0]*12
    Hr[11] = n_estimators/10000
    Hr[4] = (max_depth - 3) / 7
    Hr[5] = (min_child_weight - 1) / 5
    Hr[0] = (max_bin - 5) / 100
    Hr[6] = gamma * 2
    Hr[7] = (subsample - 0.6)/0.4
    Hr[8] = (colsample_bytree - 0.6)/0.4
    Hr[3] = (seed / 65535)
    Hr[10] = tree_method / 3
    Hr[1] = (KAug - 1) / 20
    Hr[2] = (AugLevel - 0.01) * 3.3

    return Hr

# На X0, Y0 определяем лучшую позицию (обычно это неполные наборы - часть отдана для валидации.
# Потом заново обучение на X1, Y1, если они указаны.
def BoostHr(Hr, X0, Y0, XValid, YValid, X1 = None, Y1 = None, XTest = None, SeedRandom = True, StartPos = 500, WinSize=500, GPU=0, UseMetric="mlogloss", Verbose = 0): #test_pred, accuracy, y_pred, y_pred_cat
        ...
        K = 33#1 + int(Hr[1] * 20)
        rnd = int(Hr[4] * 65535) if SeedRandom else None
        LevelK = 0.2#0.01 + Hr[2] / 3.3

        #random.seed(rnd)

        #X01, Y01 = TimeAugmentation(X0, Y0.reshape((len(Y0), 1)),
        #                              K=K, random=rnd, LevelK=LevelK, UseMain=True)  # Hr[13] > 0.9)

        #X1, Y1 = TimeAugmentation(X1, Y1.reshape((len(Y1), 1)),
        #                               K=K, random=rnd, LevelK=LevelK, UseMain=True)

        #X1Valid, Y1Valid = TimeAugmentation(X1, Y1.reshape((len(Y1), 1)),
        #                          K=1, random=rnd, LevelK=LevelK, UseMain=False)

        #ValidX, ValidY = TimeAugmentation(XValid, YValid, K=20, random=24, LevelK=0.05, UseMain=True)


        eval_set = [(XValid, YValid)]

        '''model = XGBClassifier(learning_rate=0.03,
                              n_estimators=10000, #int(Hr[11] * 10000),
                              ...
                              nthread=8,
                              scale_pos_weight=1,
                              seed=int(Hr[3] * 65535) if SeedRandom else None,
                              tree_method=CrMethod,
                              random_state=int(Hr[3] * 65535) if SeedRandom else None,
                              verbose=0
                              , silent=True)
        '''
        model = XGBClassifier(learning_rate=0.03,
        ...
        model.fit(X0, Y0, eval_metric=UseMetric, eval_set=eval_set, callbacks=[es], verbose = Verbose)
        BestIter = model.best_iteration
        # make predictions for test data
        test_pred = model.predict(XValid)
        ...
        '''model = XGBClassifier(learning_rate=0.03,
                              n_estimators=BestIter, #int(Hr[11] * 10000),
                              max_depth=int(Hr[4] * 7) + 3,
                              min_child_weight=int(Hr[5] * 5) + 1,
                              max_bin=int(Hr[0] * 100) + 5,
                              gamma=Hr[6] / 2,
        ...
        model.fit(X1, Y1, eval_metric=UseMetric, eval_set=[(X1Valid, Y1Valid)], callbacks=[es], verbose=Verbose)
        BestIter = model.best_iteration
        # make predictions for test data
